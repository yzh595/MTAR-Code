...
# [Check 1] 确保导入的是最新的 Two-Phase 模型
...
def get_epoch_drop_rate(current_epoch, total_epochs, max_rate=0.5):
    """
    Curriculum Learning 策略 (两阶段 - 阶梯式修改版):
    1. Phase 1 (0-50%): 保持 max_rate (例如 0.5)，强迫模型学习重构。
    2. Phase 2 (50%-100%): 直接突变到 0 (全序列微调)，删除中间的线性下降过程。
    """
    # 设定切换的分界点
    # 原逻辑中 Phase 3 是从 50% 开始的，所以这里设为 0.5。
    # 如果你想严格按照文档描述的 Phase 1 结束点，可以将 0.5 改为 0.4
    switch_epoch = int(total_epochs * 0.80)
    
    if current_epoch < switch_epoch:
        # Phase 1: 保持高 drop rate
        return max_rate
    else:
        # Phase 2: 突变为 0，进入完全微调模式
        return 0.0
...
def main(args):
    ...
    model_kwargs = dict(
        ...
        contrast_num_samples=args.contrast_num_samples, # <--- 新增传入
    ...
    if not args.no_compile:
        # PyTorch 2.2's DDP graph partitioner is fragile when a compiled model
        # switches between sparse and full symbolic sequence shapes. This
        # matches the proven GPU0 Drop -> NoDrop training configuration while
        # keeping Dynamo/AOTAutograd/Inductor compilation enabled.
        torch._dynamo.config.optimize_ddp = False
        if args.inductor_fallback_random:
            torch._inductor.config.fallback_random = True
            # PyTorch 2.2.1 registers identical SDPA train/inference patterns
            # when random ops fall back, which trips a duplicate-pattern
            # assertion. Keep SDPA as an ATen call and compile the surrounding
            # deterministic graph instead.
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # [Check 4] 动态计算并更新 Drop Rate
        current_drop_rate = get_epoch_drop_rate(epoch, args.epochs, max_rate=args.max_drop_rate)
            
        # Access the uncompiled module only for EMA/checkpoint state.
        raw_model = model.module
        if hasattr(raw_model, '_orig_mod'):
            raw_model = raw_model._orig_mod

        # =================================================================
        # [关键修正] 如果 Drop Rate 为 0，必须告诉 Dataset 停止读取硬盘！
        # =================================================================
        ...
        # [Check 5] 鲁棒的 Data Loader 解包
        for batch in loader:
            # 1. 解包：s_cpu 暂时留在内存，不立即上 GPU
            s_cpu = None
            if len(batch) == 3:
                x, y, s_cpu = batch 
            else:
        ...
        # Save one exact optimizer-bearing checkpoint immediately before the
        # first NoDrop epoch.  This enables compile/no-compile branching from
        # identical model, EMA, optimizer, scaler, sampler epoch and step.
        next_epoch = epoch + 1
